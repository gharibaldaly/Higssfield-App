...
- **Hosting:** the studio runs on its own Supabase project (`higgsfield-studio`, Frankfurt), fully separate from the `drsecret-ops` orders project, which is never touched. Vercel functions run in `fra1` next to the database.
